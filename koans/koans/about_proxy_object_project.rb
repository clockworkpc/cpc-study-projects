...
class Proxy
  attr_reader :instance_methods, :messages
  def initialize(target_object)
    @object = target_object
    @instance_methods = target_object.methods - Object.methods
    @messages = []
  end

  def send_and_record(my_method)
    @messages << my_method
    @object.send(my_method)
  end

  def called?(my_method)
    @messages.include?(my_method)
  end

  def number_of_times_called(my_method)
    @messages.find_all { |m| m == my_method }.count
  end

  def channel
    send_and_record(__method__)
  end
  ...
  def split(delimiter = nil)
    @messages << __method__
    @object.to_s.split(delimiter)
  end
end
